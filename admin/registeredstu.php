...
if(isset($_POST['email']))
{
  ...
  $fname=$_POST['fname'];
  $lname=$_POST['lname'];
  $sql="UPDATE `student` SET `fname`='$fname', `lname`='$lname' WHERE `email`='$email';";
...
$pageTitle = "Manage Registered Students: Welcome to Campus Recruitment Portal";
include_once $_SERVER['DOCUMENT_ROOT'] . '/campus/partials/_template.php';
echo "<script>const rows=".json_encode($rows)."</script>";
?>
<div class="container mt-5">
  <h2 class="text-center mb-3"><strong>Student Details</strong></h2>
  <?php if($updated):?>
          ...
          <tr>
            ...
            <?php for ($i = 0; $i < 3; $i++) : ?>
              <?php if ($row[$i] == null || $row[$i] == "")
                $row[$i] = "-";
              ?>
              <td> <?php echo $i==2?$row[$i]:ucwords($row[$i]) ?></td>
            <?php endfor ?>
            <td><input class="form-control" type="password" value="<?php echo $row[3] ?>" aria-label="readonly input example" readonly style="width:auto;;display:inline">
