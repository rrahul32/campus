...
if (isset($_POST['type']))
    if ($_POST['type'] == "update") {
        ...
        $jid = $_POST['jid'];
        $sql1 = "DELETE FROM `appstatus` WHERE `appid`=(SELECT `appid` FROM `applied` WHERE `jid`=$jid)";
        $sql2 = "DELETE FROM `applied` WHERE `jid`=$jid";
        $sql3 = "DELETE FROM `job` WHERE `jid`=$jid";
...
$sql = "SELECT `jname`,`jdesc`,`jdate`,`vacancy_no`,`salary`, (SELECT COUNT(`appid`)FROM `applied` WHERE `applied`.`jid`=`job`.`jid`)AS count,`jid` FROM `job` WHERE `cid`=$cid ORDER BY `jdate` DESC;";
$result = mysqli_query($conn, $sql);
...
<div class="col-9 mx-auto mt-3 border p-4 text-center justify-content-center" style="background-color:#bfdee3;">
    ...
    <div class="row">
        ...
        if ($deleted)
            echo "<div class='alert alert-dismissible alert-danger text-center col-10 mx-auto bg-light' role='alert'>Job details deleted successfully!
                <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
                </div>";
        ?>
    </div>
    <div class="row m-3 p-3 justify-content-center overflow-auto">
        <?php
        if ($rows == null)
            echo "
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row mb-3 justify-content-center">
                            <div class="col text-center">
                                <button type="submit" class="btn btn-primary" name="submit" id="submit" onclick="submitform()" value="update">
                                    Update
                                </button>
                            </div>
                            <div class="col text-center">
                                <button type="submit" class="btn btn-primary" name="delete" id="delete" onclick="submitform()" value="delete">
                                    Delete
                                </button>
                            </div>
                        </div>
...
<script>
    ...
    //submit form
    function submitform() {
        //console.log(this.event.srcElement.value)
        var form = document.forms['postjob']
        form['type'].value = this.event.srcElement.value;
        if (confirm(`Are you sure you want to ${this.event.srcElement.value} the job?`))
            form.submit();
    }
    //submit form end
    //set modal
